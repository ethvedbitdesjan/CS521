...
import numpy as np
from model_utils import Normalize, Average
...
class IntervalLinear(nn.Module):
    def __init__(self, layer: nn.Linear, mur_computation=True):
        super(IntervalLinear, self).__init__()
        self.W = layer.weight.t()
        self.b = layer.bias
        self.mur_computation = mur_computation
        
    def forward(self, x: Interval) -> Interval:
        if self.mur_computation:
            center = (x.lower + x.upper) / 2
            radius = (x.upper - x.lower) / 2
            mu_W = torch.matmul(center, self.W) + self.b
            r_W = torch.matmul(radius, torch.abs(self.W))
            
            final_lower = mu_W - r_W
            final_upper = mu_W + r_W
            return Interval(final_lower, final_upper)
        #less efficient, 4 matrix multiplications
...
class IntervalAverage(nn.Module):
    def __init__(self, dim=1, pad_idx=None):
        super(IntervalAverage, self).__init__()
        self.avg_dim = dim
        
    def forward(self, x: Interval, pad_mask) -> Interval:
        if pad_mask is None:
            return Interval(x.lower.mean(dim=self.avg_dim), x.upper.mean(dim=self.avg_dim))
        

        x.lower = x.lower * pad_mask.unsqueeze(-1).float()
        x.upper = x.upper * pad_mask.unsqueeze(-1).float()
        denominator = pad_mask.sum(dim=self.avg_dim, keepdim=True) + 1e-8
        return Interval(x.lower.sum(dim=self.avg_dim) / denominator, x.upper.sum(dim=self.avg_dim) / denominator)
    
class IntervalModel(nn.Module):
    def __init__(self, model: nn.Sequential, mur_computation=True):
        #We convert sequential model to interval model by overriding the forward function
        super(IntervalModel, self).__init__()
        self.model = nn.ModuleList(self.convert2modulelist(model, mur_computation))
        
    def convert2modulelist(self, model: nn.Module, mur_computation=True):
        layers = []
        for layer in model.children():
            if isinstance(layer, nn.Sequential):
                layers.extend(self.convert2modulelist(layer))
            elif isinstance(layer, nn.Linear):
                layers.append(IntervalLinear(layer, mur_computation=mur_computation))
            elif isinstance(layer, nn.ReLU):
                layers.append(IntervalReLU())
            elif isinstance(layer, Normalize):
                layers.append(Normalize())
            elif isinstance(layer, Average):
                layers.append(IntervalAverage(layer.avg_dim))
    ...
    def forward(self, x, absorb_last=False, pad_mask=None, skip_layers=0):
        ...
        for layer in self.model[skip_layers:-1]:
            if isinstance(layer, IntervalAverage):
                x = layer(x, pad_mask)
                continue
            x = layer(x)
